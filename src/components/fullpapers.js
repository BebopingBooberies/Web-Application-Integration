 ...
 * Fullpaper Page
 *
 * This page shows all the papers which relate to the track Fullpaper.
...
function Fullpaper(props) {
    ...
    const fullpapers = (value) => value.short_name === "fullpapers";

    const allPapers = props.papers.filter(searchPapers).filter(awardStatus).filter(fullpapers).map(
